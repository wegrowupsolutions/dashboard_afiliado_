...
export const useDocuments = () => {
  ...
  const handleDeleteDocument = async (id: number, title: string) => {
    try {
      ...
      const response = await fetch('https://webhook.serverwegrowup.com.br/webhook/excluir-arquivo-rag', {
        method: 'POST',
        headers: {
  ...
  const clearAllDocuments = async () => {
    try {
      ...
      const response = await fetch('https://webhook.serverwegrowup.com.br/webhook/excluir-rag', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
